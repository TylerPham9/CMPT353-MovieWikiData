...
def make_custom_map(category):
    ...
    :param category: string, name of the category from transform_download.py
    :return:
    """

    wd = spark.read.parquet(sys.argv[1])
    label_map = wd.filter(
        wd.label.isNotNull()).select(wd['id'].alias('wikidata_id'), wd['label'])
    data = wd.select(
